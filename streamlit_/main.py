...
with st.sidebar:
    st.session_state.page = option_menu(
        ...
        options=["my_book", "enroll_book", "pic_upload", "recommend_book"],  # 메뉴 옵션
        icons=["book", "pencil", "image", "star"],  # 아이콘 (선택 사항)
        menu_icon="cast",  # 메뉴 아이콘 (선택 사항)
        default_index=["my_book", "enroll_book", "pic_upload", "recommend_book"].index(st.session_state.page),  # 기본 선택 인덱스
    )
